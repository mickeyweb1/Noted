...
export default function StudentSetting() {
    // Fixed: Changed userInfo to user
    const { user, updateUserField, logout } = useUserContext();

    // 1. PROFILE STATE (Initialized from your context)
    const [firstName, setFirstName] = useState(user.signup?.firstName || user.fullName?.split(' ')[0] || "");
    const [lastName, setLastName] = useState(user.signup?.lastName || user.fullName?.split(' ')[1] || "");
    const [email, setEmail] = useState(user.email || "");
    const [phone, setPhone] = useState(""); // Mock phone number for UI
    ...
    const handleSaveProfile = () => {
        // Update your context with the new values
        updateUserField('signup', 'firstName', firstName);
        updateUserField('signup', 'lastName', lastName);
        updateUserField('signup', 'email', email);
        
        // TODO: Later, you will add your database API call here
