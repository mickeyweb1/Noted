...
export default function StudentHome() {
  ...
  const [stats, setStats] = useState({ streak: 0, totalNotes: 0, totalQuizzes: 0 });
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-brand-soft via-accent/40 to-electric-soft border border-border p-5 shadow-soft sm:p-6 lg:rounded-3xl lg:p-8">
        ...
        <div className="relative z-10 flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-end">
          ...
          <div className="flex w-full flex-col gap-3 sm:flex-row lg:w-auto">
            <NavLink to="/aiGenerator" className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-brand px-4 text-sm font-semibold text-brand-foreground shadow-lg shadow-brand/20 transition-all hover:scale-105 hover:bg-brand/90 active:scale-95 sm:w-auto sm:px-6">
                  ...
                  <NavLink key={item._id} to="/myLibrary" className="group flex min-w-0 items-center justify-between gap-3 p-4 transition-colors hover:bg-accent/50 sm:p-5">
                    <div className="flex min-w-0 items-center gap-3">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${item.type === "quiz" ? "bg-electric/10 text-electric" : "bg-brand/10 text-brand"}`}>
                        {item.type === "quiz" ? <Brain className="w-5 h-5" /> : <BookOpen className="w-5 h-5" />}
                      </div>
                ...
                {pendingBattles.slice(0, 1).map((battle) => (
                  <div key={battle._id} className="p-3 rounded-lg bg-card border border-brand/20 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-semibold text-foreground">{battle.challengerName}</span>
            ...
            <div className="space-y-3">
              {isLoading ? (
                ...
                topStudents.map((student, index) => (
                  <div key={student._id} className="flex items-center gap-3 p-2.5 rounded-lg bg-muted/30 border border-border hover:bg-muted/50 transition-colors">
                    ...
                    <span className="text-sm font-bold text-brand shrink-0">{student.xp} XP</span>
                  </div>
                ))
              ) : (
