...
import { Brain, FileText, Plus, Trash2, Sparkles, Loader2, CheckCircle2, Copy, Users, Clock, BarChart3 } from "lucide-react";
...
export default function AdminQuizGenerator() {
  ...
  const addManualQuestion = () => {
    setManualQuestions([...manualQuestions, { question: "", options: ["", "", "", ""], correctAnswer: "", explanation: "" }]);
  };
  ...
  // ✅ FIXED: Auto-complete now properly updates the UI with the generated quiz and codes
  const handleAutoComplete = async () => {
    ...
    try {
      // This endpoint generates the remaining questions, saves the quiz, and returns the codes
      ...
      // Show the success view with the codes
      setGeneratedQuiz(res.data.data);
      setAccessCodes(res.data.accessCodes);
      setActiveTab("ai"); // Reuses the success view to display the codes cleanly
    } catch (err) {
  ...
  const handleCreateManualQuiz = async () => {
    ...
    try {
      ...
      setGeneratedQuiz(res.data.data);
      setAccessCodes(res.data.accessCodes);
      setActiveTab("ai"); // Switch to view the generated codes
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input type="text" placeholder="Quiz Title (e.g., Biology Chapter 1)" value={metadata.title} onChange={(e) => handleMetadataChange("title", e.target.value)} className="border border-border rounded-lg p-3 focus:ring-2 focus:ring-brand focus:outline-none md:col-span-2" />
            ...
            <div className="flex items-center gap-2 border border-border rounded-lg p-3">
              ...
              <select value={metadata.timeType} onChange={(e) => handleMetadataChange("timeType", e.target.value)} className="text-sm text-muted-foreground focus:outline-none">
                <option value="total">mins total</option>
                <option value="perQuestion">mins per Q</option>
              </select>
            </div>

            <input type="number" min="1" value={metadata.numQuestions} onChange={(e) => handleMetadataChange("numQuestions", Number(e.target.value))} className="border border-border rounded-lg p-3 focus:ring-2 focus:ring-brand focus:outline-none" placeholder="Number of Questions" />
            <input type="number" min="1" value={metadata.numStudents} onChange={(e) => handleMetadataChange("numStudents", Number(e.target.value))} className="border border-border rounded-lg p-3 focus:ring-2 focus:ring-brand focus:outline-none" placeholder="Number of Students (Codes)" />
            {/* ✅ FIXED: Removed the stray "each" text that was here */}
          </div>
        </div>
        ...
        {activeTab === "ai" && !generatedQuiz && (
          <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={8} placeholder="Paste your notes, textbook text, or lecture summary here. The AI will generate questions based on this content." className="w-full border border-border rounded-lg p-4 focus:ring-2 focus:ring-brand focus:outline-none" />
        ...
        {activeTab === "manual" && !generatedQuiz && (
          <div className="space-y-4">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={handleAutoComplete} disabled={isAutocompleting} className="flex-1 py-3 bg-electric/10 text-electric border border-electric/20 rounded-xl font-semibold hover:bg-electric/20 transition flex items-center justify-center gap-2 disabled:opacity-50">
                {isAutocompleting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} AI Auto-Complete & Create
              </button>
            </div>
            <button onClick={handleCreateManualQuiz} className="w-full py-4 bg-brand text-brand-foreground rounded-xl font-bold text-lg hover:bg-brand/90 transition">Create Quiz & Generate Codes</button>
          </div>
        )}
        ...
        {generatedQuiz && (
          <div className="bg-card border border-green-500/30 bg-green-500/5 rounded-2xl p-6 space-y-6 animate-in fade-in">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => { setGeneratedQuiz(null); setAccessCodes([]); setManualQuestions([{ question: "", options: ["", "", "", ""], correctAnswer: "", explanation: "" }]); setNotes(""); }} className="text-sm text-muted-foreground hover:text-foreground">Create Another</button>
            </div>
            
            <div className="bg-background rounded-xl p-4 border border-border">
              <h3 className="font-semibold mb-2">Quiz: {generatedQuiz.title}</h3>
              <p className="text-sm text-muted-foreground">{generatedQuiz.questions.length} Questions • {metadata.difficulty} • {metadata.timeLimit} mins ({metadata.timeType})</p>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-2 max-h-60 overflow-y-auto p-2 bg-background rounded-xl border border-border">
                {accessCodes.map((code, idx) => (
                  <div key={idx} className="text-center font-mono text-sm bg-muted p-2 rounded border border-border">{code}</div>
                ))}
              </div>
        ...
        {activeTab === "results" && (
          <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
            ...
            <div className="flex gap-4">
              <input type="text" placeholder="Paste Quiz ID to search" value={selectedQuizId} onChange={(e) => setSelectedQuizId(e.target.value)} className="flex-1 border border-border rounded-lg p-3 focus:ring-2 focus:ring-brand focus:outline-none" />
              <button onClick={fetchResults} className="px-6 py-3 bg-brand text-brand-foreground rounded-lg font-semibold hover:bg-brand/90">Fetch Results</button>
            </div>
